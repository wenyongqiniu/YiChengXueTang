...
import com.example.yichengxuetang.adapter.MuluSimluListAdapter;
import com.example.yichengxuetang.adapter.MuluSlimuTitleAdapter;
import com.example.yichengxuetang.adapter.QuestionAnswerAdapter;
...
import static com.example.yichengxuetang.adapter.SmartDohomeworkAdapter.rl_anlisy;
import static com.example.yichengxuetang.application.MyApplication.getActivityManager;

public class OnlyActivity extends MvpActivity<QuestionBankContract.QuestionBankPresenter> implements QuestionBankContract.QuestionBankView {

    private int examType = 1;
    public static CustomViewPager mVp;
    private SmartDohomeworkAdapter welcomeAdapter;
    public static ArrayList<QuestionListResponse.DataBean.DataListBean> questionList;
    ...
    public static ChjTimer chjTimer;//计时器
    ...
    private int useTime;
    public static QuestionMuluResponse.DataBean dataMulu;
    ...
    public void getQuestionMulu(QuestionMuluResponse wallPaperResponse) {
        if (wallPaperResponse.getCode() == 0) {
            dataMulu = wallPaperResponse.getData();
            new XPopup.Builder(context)
                    .moveUpToKeyboard(false) //如果不加这个，评论弹窗会移动到软键盘上面
                    .enableDrag(true)
                    .maxHeight(ScreenUtils.getScreenHeight(context) * 3 / 4)
                    .asCustom(new MuluPopup(context, dataMulu, number))
                    .show();
        }
    ...
    public void initData(Bundle savedInstanceState) {
        ...
        welcomeAdapter = new SmartDohomeworkAdapter(questionList, context, mPresenter, questionTitle);
        // ViewPager2Adapter welcomeAdapter1 = new ViewPager2Adapter(questionList, context, mPresenter, questionTitle);
    ...
    public class MuluSimluPopup extends BottomPopupView {
        ...
        protected void onCreate() {
            ...
            rv_mulu_title.setLayoutManager(linearLayoutManager);
    ...
    public static class InterruptOnlyPopup extends BottomPopupView {
        ...
        protected void onCreate() {
            ...
            //查看错题
            tv_find_error_questioin.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View v) {
                    dismiss();
                    dissmiss = 1;//表示不退出页面
